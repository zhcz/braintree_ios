#import <XCTest/XCTest.h>
#import <BraintreeVisaCheckout/BraintreeVisaCheckout.h>
#import <UIKit/UIKit.h>
#import <VisaCheckoutFramework/VisaCheckoutFramework.h>

#define kTokenizationKeyWithVisaCheckout SANDBOX_TOKENIZATION_KEY
#define kTokenizationKeyWithoutVisaCheckout @"development_testing_altpay_sandbox_merchant"

@interface BraintreeVisaCheckout_IntegrationTests : XCTestCase
@property (nonatomic, strong) BTVisaCheckoutClient* visaCheckoutClient;
@end

@implementation BraintreeVisaCheckout_IntegrationTests

- (void)setUp {
    [super setUp];
    
    BTAPIClient *apiClient = [[BTAPIClient alloc] initWithAuthorization:kTokenizationKeyWithVisaCheckout];
    self.visaCheckoutClient = [[BTVisaCheckoutClient alloc] initWithAPIClient:apiClient];
}

- (VisaPaymentSummary *) sampleVisaPaymentSummary {
    VisaPaymentSummary *visaPaymentSummary = [[VisaPaymentSummary alloc] init];
    visaPaymentSummary.encKey = @"FXVZWGhvZed5jAUCXuoiCVcsD0Eb6lvlwjAIE7Bxf/uMpQLT5g/Tcwhk3ujfl1zdQeUvJzdy+MMk+k7tjUZ9wtx+cknhHVni10yfFmkPgIblceq/YiQ0Kdea14fZxa1+";
    visaPaymentSummary.encPaymentData = @"";
        visaPaymentSummary.callId = @"5318456373519624201";
    return visaPaymentSummary;
}

- (void)testTokenizeVisaPaymentSummary_withValidVisaPaymentSummary_isSuccessful {
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback invoked"];
    [self.visaCheckoutClient tokenizeVisaPaymentSummary:[self sampleVisaPaymentSummary] completion:^(BTVisaCheckoutCardNonce * _Nullable tokenizedVisaCheckoutCard, NSError * _Nullable error) {
        XCTAssertNotNil(tokenizedVisaCheckoutCard);
        XCTAssertNil(error);
        [expectation fulfill];
        
    }];
    
    [self waitForExpectationsWithTimeout:5 handler:nil];
}

- (void)pendTokenizeVisaPaymentSummary_withInvalidPaymentSummary_isNotSuccessful {
    VisaPaymentSummary *visaPaymentSummary = [[VisaPaymentSummary alloc] init];
    visaPaymentSummary.callId = @"callId";
    visaPaymentSummary.encPaymentData = @"encryptedPayload";
    visaPaymentSummary.encKey = @"encKey";
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback invoked"];
    [self.visaCheckoutClient tokenizeVisaPaymentSummary:visaPaymentSummary completion:^(BTVisaCheckoutCardNonce * _Nullable tokenizedVisaCheckoutCard, NSError * _Nullable error) {
        XCTAssertNotNil(error);
        XCTAssertNil(tokenizedVisaCheckoutCard);
        [expectation fulfill];
        
    }];
    
    [self waitForExpectationsWithTimeout:5 handler:nil];
}

- (void)testTokenizeVisaPaymentSummary_withVisaPaymentInfo_isNotSuccessful {
    VisaPaymentInfo *visaPaymentInfo = [[VisaPaymentInfo alloc] init];
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback invoked"];
    [self.visaCheckoutClient tokenizeVisaPaymentSummary:visaPaymentInfo completion:^(BTVisaCheckoutCardNonce * _Nullable tokenizedVisaCheckoutCard, NSError * _Nullable error) {
        XCTAssertNotNil(error);
        XCTAssertTrue([@"A valid Visa payment summary is required." isEqualToString:error.localizedDescription]);
        XCTAssertNil(tokenizedVisaCheckoutCard);
        [expectation fulfill];
    }];
    
    [self waitForExpectationsWithTimeout:5 handler:nil];
}

- (void)testTokenizeVisaPaymentSummary_whenVisaCheckoutNotEnabled_isNotSuccessful {
    BTAPIClient *apiClient = [[BTAPIClient alloc] initWithAuthorization:kTokenizationKeyWithoutVisaCheckout];
    BTVisaCheckoutClient *badVisaCheckoutClient = [[BTVisaCheckoutClient alloc] initWithAPIClient:apiClient];
    
    XCTestExpectation *expectation = [self expectationWithDescription:@"Callback invoked"];
    [badVisaCheckoutClient tokenizeVisaPaymentSummary:[self sampleVisaPaymentSummary] completion:^(BTVisaCheckoutCardNonce * _Nullable tokenizedVisaCheckoutCard, NSError * _Nullable error) {
        XCTAssertNotNil(error);
        XCTAssertTrue([@"Visa Checkout is not enabled for this merchant. Please ensure that Visa Checkout is enabled in the Braintree Control Panel and try again." isEqualToString:error.localizedDescription]);
        XCTAssertNil(tokenizedVisaCheckoutCard);
        [expectation fulfill];
    }];
    
    [self waitForExpectationsWithTimeout:5 handler:nil];
    
}


@end
